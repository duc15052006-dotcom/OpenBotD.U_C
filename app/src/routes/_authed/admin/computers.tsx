...
import { agentListQueryOptions, type ComputerResourceProfile } from "@/lib/agents/queries";
import {
...
export function computerStartWarning(
  ...
  if (capacity.memoryBytes && capacity.memoryBytes > 0) {
    ...
    if (projected >= capacity.memoryBytes * 0.8) {
      return `Computer memory quotas would reach about ${Math.round(
        (projected / capacity.memoryBytes) * 100,
      )}% of the container engine's available RAM.`;
    }
  }
  ...
  if (capacity.logicalCpus && capacity.logicalCpus > 0) {
    ...
    if (projectedNanoCpus >= hostNanoCpus * 0.8) {
      return `Computer CPU quotas would reach about ${Math.round(
        (projectedNanoCpus / hostNanoCpus) * 100,
      )}% of the container engine's logical CPU capacity.`;
    }
  }
