...
function fleet(
  ...
  return {
    computers: runningIds.map((botId) => ({ botId, running: true })),
    capacity: {
      memoryBytes: 16 * 1024 ** 3,
      logicalCpus: 12,
      maxActiveComputers: 3,
      resourceProfiles: quotas,
      ...overrides,
    },
  };
}
...
describe("Computer start resource warning", () => {
  test("warns before consuming the final active Computer slot", () => {
    expect(
      computerStartWarning(fleet(["a", "b"]), "c", {
        a: "normal",
        b: "normal",
        c: "light",
      }),
  ...
  test("uses the target and running Agent profiles for projected memory", () => {
    expect(
  ...
  test("uses per-Agent CPU quotas instead of assuming Normal", () => {
    expect(
      computerStartWarning(
        fleet(["a"], {
          maxActiveComputers: 10,
  ...
  test("does not warn when the requested profile fits comfortably", () => {
    expect(
      computerStartWarning(
        fleet(["a"], {
